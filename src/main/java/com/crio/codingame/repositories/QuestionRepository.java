...
public class QuestionRepository implements IQuestionRepository {
    ...
    public List<Question> findAll() {
      List<Question> list = questionMap.entrySet().stream().map(Map.Entry::getValue)
    ...
    public List<Question> findAllQuestionLevelWise(Level level) {
        ...
        List<Question> questions=findAll().stream().filter(c->c.getLevel().equals(level)).collect(Collectors.toList());
     return questions;
    }
